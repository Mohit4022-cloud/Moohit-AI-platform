...
import { BarChart3, Users, Radio, MessageSquare, TrendingUp, Settings, Sparkles, Phone, Mail, Clock, ChevronRight, Activity, Target, Zap, Brain, Eye, Mic, Command, AlertCircle, CheckCircle, ArrowUp, ArrowDown, TrendingDown, Calendar, Shield, Headphones, Volume2, Keyboard, X } from 'lucide-react';
...
const EnhancedLeadCard = ({ lead, index, aiScore, onAction }) => {
        ...
        <div className="lead-actions">
          ...
          {quickActions && (
            <div className="quick-actions">
              <button className="quick-action" title="Call" onClick={(e) => {
                e.stopPropagation();
                onAction(lead.id, 'call');
              }}>
                <Phone size={14} />
              </button>
              <button className="quick-action" title="Email" onClick={(e) => {
                e.stopPropagation();
                onAction(lead.id, 'email');
              }}>
                <Mail size={14} />
              </button>
              <button className="quick-action" title="Chat" onClick={(e) => {
                e.stopPropagation();
                onAction(lead.id, 'chat');
              }}>
      ...
      {isExpanded && (
        <div className="lead-expanded">
          ...
          <div className="lead-actions-bar">
            <button className="primary-action" onClick={(e) => {
              e.stopPropagation();
              onAction(lead.id, 'call');
            }}>
              <Phone size={14} />
            ...
            <button className="secondary-action" onClick={(e) => {
              e.stopPropagation();
              onAction(lead.id, 'schedule');
            }}>
              <Calendar size={14} />
              Schedule
            </button>
...
export default function QuantumDashboard() {
  ...
  const [activeFilter, setActiveFilter] = useState('hot-leads');
  ...
  const handleNavigation = (navId) => {
    ...
    if (navId === 'leads') {
      ...
      navigate('/live-queue');
    } else if (navId === 'conversations') {
      navigate('/conversations');
    } else if (navId === 'analytics') {
      navigate('/analytics');
    } else if (navId === 'campaigns') {
      navigate('/campaigns');
    } else if (navId === 'settings') {
      navigate('/settings');
    }
    showToast(`Navigating to ${navId}`, 'success');
  };
  
  // Lead action handlers
  const handleLeadAction = (leadId, action) => {
    setLeadActionStatus({ ...leadActionStatus, [leadId]: action });
    switch(action) {
      case 'call':
      ...
      case 'email':
        ...
        break;
      case 'chat':
        showToast('Starting chat session...', 'info');
        break;
      case 'schedule':
        showToast('Opening calendar...', 'info');
        break;
      case 'qualify':
        showToast('Running qualification analysis...', 'info');
        setTimeout(() => showToast('Lead qualified as High Priority', 'success'), 2000);
        break;
      default:
  ...
  const handleFilterChange = (filter) => {
    ...
    showToast(`Filtering by: ${filter === 'available' ? 'Available to Call' : 'Hot Leads'}`, 'info');
  };
  ...
  const handleViewAll = () => {
    navigate('/leads');
    showToast('Viewing all leads', 'info');
  };
  
  // Today button handler
  const handleToday = () => {
    showToast('Showing today\'s activities', 'info');
    ...
    <div className={`quantum-dashboard ${focusMode ? 'focus-mode' : ''}`} style={adaptiveStyles}>
      ...
      <aside className="quantum-sidebar">
        ...
        <div className="sidebar-footer">
          ...
          <button className="quantum-button quantum-button--secondary" onClick={handleTryAI}>
            <Sparkles />
            Try AI
      ...
      <main className="quantum-main">
        <header className="main-header">
          ...
          <div className="header-actions">
            ...
            <button className="icon-button" title="Keyboard Shortcuts" onClick={handleKeyboardShortcuts}>
              <Keyboard size={20} />
            </button>
            
            <div className="notification-wrapper">
              <button className="icon-button notification-button" onClick={handleNotifications}>
                <Activity size={20} />
              ...
              {showNotifications && (
                <div className="notifications-dropdown">
                  ...
                  <div className="notification-item">
                    <AlertCircle size={16} className="notification-icon info" />
                    <div>
                      <p>New hot lead assigned</p>
                      <span className="notification-time">5 minutes ago</span>
                    </div>
                  </div>
                  <div className="notification-item">
                    <TrendingUp size={16} className="notification-icon success" />
                    <div>
                      <p>Conversion rate up 15%</p>
          ...
          <section className="performance-section">
            <div className="section-header-enhanced">
              ...
              <div className="header-controls">
                ...
                <button className="control-button primary" onClick={handleQuickActions}>
                  <Zap size={16} />
                  Quick Actions
                </button>
              </div>
          ...
          <section className="leads-section">
            <div className="section-header">
              ...
              <div className="section-actions">
                ...
                <button 
                  className={`filter-button ${activeFilter === 'hot-leads' ? 'active' : ''}`}
                  onClick={() => handleFilterChange('hot-leads')}
                >
                  <Zap size={16} />
                  Hot Leads
                </button>
        ...
        <div className="modal-overlay" onClick={() => setShowQuickActionsModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3><Zap size={24} /> Quick Actions</h3>
              <button className="modal-close" onClick={() => setShowQuickActionsModal(false)}>
                ×
              </button>
            </div>
            <div className="quick-actions-grid">
              <button className="quick-action-item" onClick={() => {
                showToast('Bulk email sent to 10 leads', 'success');
              ...
              <button className="quick-action-item" onClick={() => {
                showToast('Auto-dialer started', 'success');
                setShowQuickActionsModal(false);
              }}>
                <Phone size={24} />
                <span>Auto Dial</span>
              ...
              <button className="quick-action-item" onClick={() => {
                showToast('Lead scoring updated', 'success');
                setShowQuickActionsModal(false);
              }}>
                <Target size={24} />
                <span>Re-score Leads</span>
              </button>
              <button className="quick-action-item" onClick={() => {
      ...
      {showAIModal && (
        <div className="modal-overlay" onClick={() => setShowAIModal(false)}>
          <div className="modal-content ai-modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3><Brain size={24} /> AI Assistant</h3>
      ...
      {showKeyboardShortcuts && (
        <div className="modal-overlay" onClick={() => setShowKeyboardShortcuts(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3><Keyboard size={24} /> Keyboard Shortcuts</h3>
              <button className="modal-close" onClick={() => setShowKeyboardShortcuts(false)}>
                ×
              </button>
            </div>
            ...
            <button className="close-button" onClick={() => setShowKeyboardShortcuts(false)}>
              Close
            </button>
          </div>
        </div>
